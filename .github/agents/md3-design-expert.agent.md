...
mcp-servers:
  serena:
    type: local
    command: uvx
    args: ["--from", "serena-agent", "serena", "start-mcp-server", "--context", "ide-assistant"]
  repowise:
    type: local
    command: repowise
    args: ["mcp", "."]
...
2. **Inspect**: Screenshot the live app (`http://localhost:8081`) using browser tools. Read relevant source files only when visual evidence is insufficient.
...
- Never make a design claim without first checking — screenshot the app or read the source. No guessing.
